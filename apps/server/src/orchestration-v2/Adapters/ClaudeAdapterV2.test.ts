...
describe("ClaudeAdapterV2 background wake turns", () => {
  ...
  it.effect("keeps a background subagent's text and tools on its child thread across a wake", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const TASK_ID = "task-routed-subagent";
        const TOOL_USE_ID = "toolu-routed-subagent";
        const FINAL_REPORT = "Audit complete: nothing to report.";
        const subagentAssistant = (input: {
          readonly uuid: string;
          readonly text?: string;
          readonly bashToolUseId?: string;
        }) => makeSubagentAssistantFrame({ ...input, parentToolUseId: TOOL_USE_ID });
        const subagentToolResult = (input: { readonly uuid: string; readonly toolUseId: string }) =>
          makeSubagentToolResultFrame({ ...input, parentToolUseId: TOOL_USE_ID });
        ...
        yield* harness.runtime.startTurn(
          makeClaudeTestTurnInput({
            ...
            providerThread: harness.providerThread,
            now,
            attemptId: RunAttemptId.make("attempt-claude-routed-1"),
            text: "Spawn a background auditor and stop.",
            attachments: [],
          }),
        );
        yield* Queue.offer(
        ...
        // Live while the root turn is active.
        yield* Queue.offer(
          harness.sdkMessages,
          subagentAssistant({
            uuid: "00000000-0000-4000-8000-000000000302",
            text: "Reading the log first.",
            bashToolUseId: "toolu-sub-bash-1",
          }),
        );
        yield* Queue.offer(
          harness.sdkMessages,
        ...
        yield* Queue.offer(
          ...
          makeResultFrame({
            uuid: "00000000-0000-4000-8000-000000000304",
            result: "The auditor is running in the background.",
          }),
        );
        yield* awaitUntil(() => harness.terminalEvents().length === 1, "first turn terminal");

        // Buffered while the root is idle, then drained into the continuation.
        yield* Queue.offer(
          harness.sdkMessages,
          subagentAssistant({
            uuid: "00000000-0000-4000-8000-000000000305",
            text: "Now the diffs.",
            bashToolUseId: "toolu-sub-bash-2",
          }),
        );
        yield* Queue.offer(
          harness.sdkMessages,
          subagentToolResult({
            ...
            toolUseId: "toolu-sub-bash-2",
          }),
        );
        yield* Queue.offer(
          harness.sdkMessages,
          subagentAssistant({ uuid: "00000000-0000-4000-8000-000000000307", text: FINAL_REPORT }),
        );
        yield* Queue.offer(
          harness.sdkMessages,
          claudeSdkFrame({
            ...
            tool_use_id: TOOL_USE_ID,
            status: "completed",
            output_file: "/tmp/task-routed-subagent.output",
        ...
        yield* harness.runtime.startTurn(
          makeClaudeTestTurnInput({
            ...
            providerThread: harness.providerThread,
            now,
            attemptId: RunAttemptId.make("attempt-claude-routed-2"),
            text: "Background task completed.",
            attachments: [],
            providerTurnOrdinal: 2,
            messageCreatedBy: "agent",
        ...
        const routing = subagentRouting(harness.events, ["toolu-sub-bash-1", "toolu-sub-bash-2"]);
        assert.isDefined(routing.childThreadId);
        assert.deepEqual(
          [...routing.toolThreadIds.entries()].map(([id, threads]) => [id, [...threads]]),
          [
            ["toolu-sub-bash-1", [routing.childThreadId]],
            ["toolu-sub-bash-2", [routing.childThreadId]],
          ],
        );
        ...
        assert.deepEqual(routing.assistantTexts(harness.threadId), [
          "The auditor is running in the background.",
